...
```mermaid
flowchart LR
  C["사실 주장<br/>수치·이름·날짜·인용"] --> Q{"6종 출처로<br/>추적되나?"}
  Q -->|"USER·READ·BASH<br/>WEB·MEMORY·MCP"| A["단정 + 출처 마커"]
  Q -->|"출처 없음"| H["보류 → 5모드 정직 보고<br/>UNKNOWN·PARTIAL·TOOL_FAILED<br/>OUT_OF_SCOPE·UNCERTAIN"]
...
### (2) 일관성 : 깨지지 않는 핵심 구조 (불변성)

이것이 하네스의 핵심 명제입니다 — 규칙은 간단명료할수록 강하지만, 어떤 상황에서도 깨지지
않아야 합니다. 깨지지 않는 소수의 핵심 규칙은 휘어지는 많은 규칙보다 강합니다. 작업·머신·
모델이 달라져도 구조가 그대로이기에(불변성 원칙), 하네스는 프로젝트를 가로지르는 *불변의
OS*처럼 작동합니다. 이 예측 가능성이 곧 (1) 할루시네이션 방지와 (3) 지식 축적이 매번 동일하게
작동하는 토대입니다.

```mermaid
flowchart LR
  A["작업이 달라도"] --> K["깨지지 않는 소수의<br/>핵심 규칙 (불변성)"]
  B["머신이 달라도"] --> K
  M["모델이 달라도"] --> K
  K --> O["언제나 동일한 행동<br/>= 불변의 OS"]
  O --> R["예측 가능성 → 신뢰<br/>→ 그 위에 (1)·(3)이 쌓임"]
...
```mermaid
flowchart LR
  W["작업 수행"] --> S{"산출물 분류<br/>(운영 데이터 누출 없이)"}
  S -->|"리서치 산출물"| K["지식 저장소"]
  S -->|"교훈"| Me["기억 (memory)"]
  S -->|"한 줄 이벤트"| L["로그"]
  K --> N["다음 작업이 더 유능"]
